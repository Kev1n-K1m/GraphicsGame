            ...
            for line in matFile:
                ...
                if len(vals) > 0 :
                    ...
                    ## load texture file info
                        ...
                        fileName = vals[1]   
                        # print("-----------------------")    
                        # self.materials.append(self.loadTexture(fileName))
                        if self.materials[-1][5] == None :
                ...
                if len(vals) > 0:
                    ...
                    ## Load the faces
                        ...
                        for v in vals[1:]:
                            ...
                            # Extract vertex index (convert to 0-based)
                            v_idx = int(vertex_data[0]) - 1
                            
                            # Extract texture coordinate index (convert to 0-based, check if present)
                            vt_idx = int(vertex_data[1]) - 1 if len(vertex_data) > 1 and vertex_data[1] != '' else None
                            
                            # Extract normal index (convert to 0-based, check if present)
                            vn_idx = int(vertex_data[2]) - 1 if len(vertex_data) > 2 else None
                            
                            # Add the tuple (vertex index, texture index, normal index) to tempFace
                            tempFace.append([v_idx, vt_idx, vn_idx])
    ...
    def drawObject(self):
        ...
        for face in self.faces:
            ...
            if face[0] == -1:
                ...
                GL.glBegin(GL.GL_POLYGON)
                # Drawing normal, then texture, then vertex coords
                for f in face:
                    # Check if normal exists and is valid (-1 or None means no normal)
                    if f[2] is not None and f[2] != -1 and self.norms != []:
                        GL.glNormal3f(self.norms[f[2]][0],
                                    self.norms[f[2]][1],
                                    self.norms[f[2]][2])

                    # Check if texture coordinates exist and are valid (-1 or None means no texture coord)
                    if f[1] is not None and f[1] != -1:
                        GL.glTexCoord2f(self.texCoords[f[1]][0],
                                        self.texCoords[f[1]][1])

                    # Always set vertex coordinates
                    GL.glVertex3f(self.verts[f[0]][0],
                                self.verts[f[0]][1],
                                self.verts[f[0]][2])
                GL.glEnd()
